...
# Load the predictor
predictor = dlib.shape_predictor("C:\\Users\\zergs\\Desktop\\LipReadingTool\\models\\face_landmarks.dat")
...
data_count = 1
words = ['I', 'need', 'help', 'please', 'you', 'speak', 'can', 'hear', 'we', 'love', 'see', 'watch', 'hello']
# label_dict = {0: 'I', 1: 'need', 2: 'help', 3: 'please', 4: 'you', 5: 'speak', 6: 'can', 7: 'hear', 8: 'we', 9: 'love', 10: 'see', 13: 'watch', 14: 'hello'}
options = ", ".join(words)
label = input("What word you like to collect data for? The options are \n" + options + ": ")
labels = []

custom_distance = input("If you want, enter a custom lip distance threshold or press enter to set to default: ")

'''
clean_output_dir = input("Clean output directory of the current word? (Y/N): ").lower()

#clear the directory if needed
if clean_output_dir == "yes" or clean_output_dir == "y":
    ...
    print("Not cleaning the directory...")
...
while True:
    ...
    cv2.imshow(winname="Lip Reading tool: Record lips", mat=frame)
...
def saveAllWords(all_words):
    ...
    ""
    output_dir = "C:\\Users\\zergs\\Desktop\\LipReadingTool\\my_outs"
    next_dir_number = 1
    for i, word_frames in enumerate(all_words):

        label = labels[i]
        # keys = str(reverse_lookup(label_dict, label)) // prints the keys of each word
        # keys = keys.translate({ord(i): None for i in '[]'})
